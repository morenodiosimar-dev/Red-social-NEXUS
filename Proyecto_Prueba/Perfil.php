    ...
    <div class="perfil" id="form-perfil">
        <div class="perfil-usuario">
            <div class="superior">
                ...
                <ion-icon name="menu-outline" class="icon-menu cursor-pointer hover:text-purple-500"
                    onclick="window.location.href='conf.php'"></ion-icon>
            </div>
        </div>
        ...
        <div class="perfil-usuario flex items-center gap-4 p-4">
            <div onclick="document.getElementById('input-foto-perfil').click()"
                ...
                <div
                    class="absolute inset-0 bg-black/10 opacity-0 hover:opacity-100 transition-opacity flex items-center justify-center">
                    <ion-icon name="camera-outline" class="text-white text-xl"
                        style="width: 30px; height: 30px;"></ion-icon>
                </div>
            </div>
        ...
        <div class="perfiles flex justify-around border-b">
            <a href="perfil.php?tab=personal"
                class="perso p-2 <?php echo $tab == 'personal' ? 'active-tab' : ''; ?>">Personal</a>
            <a href="perfil.php?tab=contenido"
                class="cont p-2 <?php echo $tab == 'contenido' ? 'active-tab' : ''; ?>">Contenido</a>
        </div>

        <div class="mt-6">
            <h3 class="px-6 text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">Mis Fotos
                (<?= ucfirst($tab) ?>)</h3>
            <div class="relative w-full overflow-hidden">
                <button onclick="window.deslizarCarrusel(-1)"
                    class="absolute left-2 top-1/2 -translate-y-1/2 z-40 bg-white/70 hover:bg-white rounded-full p-2 shadow-md">
                ...
                <button onclick="window.deslizarCarrusel(1)"
                    class="absolute right-2 top-1/2 -translate-y-1/2 z-40 bg-white/70 hover:bg-white rounded-full p-2 shadow-md">
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                <?php if ($res_reposts->num_rows > 0):
                    while ($r = $res_reposts->fetch_assoc()): ?>
                        <div
                            class="relative group bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm hover:shadow-md transition-shadow cursor-pointer">
                            <div class="relative aspect-square" onclick="abrirDetalle(<?= $r['id'] ?>)">
                                ...
                                <div
                                    class="absolute top-2 left-2 bg-green-500/90 text-white p-1.5 rounded-full text-xs shadow-lg">
        ...
        <div class="iconos-inferiores">
            ...
            <ion-icon name="search-outline" class="icon-gradient"
                onclick="window.location.href='busqueda.php'"></ion-icon>
            <ion-icon name="chatbubble-outline" class="icon-gradient"
                onclick="window.location.href='http://localhost:3000'"></ion-icon>
            <ion-icon name="person-outline" class="icon-gradient active-icon"
                onclick="window.location.href='perfil.php'"></ion-icon>
        </div>
    </div>
