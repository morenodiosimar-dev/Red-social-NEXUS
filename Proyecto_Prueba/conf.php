...
<body class="bg-gray-100">
    ...
    <div class="contenedor-conf ">
        ...
        <div class="titulo-conf ">
            <div class="conf ">
                <ion-icon name="chevron-back-outline" onclick="window.history.back()" class="devolver cursor-pointer"
                    id="btn-devolver"></ion-icon>
        ...
        <div class="info-conf">
            <div class="select-conf ">
                ...
                <div id="reacciones-section" class="mt-4 hidden">
                    ...
                    <div class="mb-4 flex gap-2 items-center">
                        ...
                        <button id="buscarReacciones"
                            class="bg-blue-500 text-white px-3 py-1 rounded hover:bg-blue-600">Buscar</button>
                    </div>

                    <!-- Botones de filtro -->
                    <div class="mb-4 flex gap-2">
                        <button id="misReacciones" class="bg-gray-200 px-3 py-1 rounded hover:bg-gray-300">Mis
                            reacciones</button>
                        <button id="reaccionesAMi" class="bg-gray-200 px-3 py-1 rounded hover:bg-gray-300">Me han
                            Reaccionado</button>
                    </div>

                    <!-- Lista -->
                    <div id="lista-reacciones"
                        class="border rounded p-2 max-h-96 overflow-y-auto text-sm text-gray-700">
                        <p class="text-gray-500">Selecciona una fecha y haz clic en "Buscar" para ver tu historial de
                            reacciones.</p>
                    </div>
                </div>
                ...
                <div id="comentarios-section" class="mt-4 hidden">
                    ...
                    <div class="mb-4 flex gap-2 items-center">
                        <input type="text" id="coment-fecha-inicio" class="border rounded px-2 py-1"
                            placeholder="Desde">
                        <input type="text" id="coment-fecha-fin" class="border rounded px-2 py-1" placeholder="Hasta">
                        <button id="buscarComentarios"
                            class="bg-purple-500 text-white px-3 py-1 rounded hover:bg-purple-600">Buscar</button>
                    </div>

                    <div class="mb-4 flex gap-2">
                        <button id="misComentarios" class="bg-gray-200 px-3 py-1 rounded hover:bg-gray-300">Mis
                            comentarios</button>
                        <button id="comentariosAMi" class="bg-gray-200 px-3 py-1 rounded hover:bg-gray-300">Me han
                            comentado</button>
                    </div>

                    <div id="lista-comentarios"
                        class="border rounded p-2 max-h-96 overflow-y-auto text-sm text-gray-700">
                        <p class="text-gray-500">Selecciona fechas y haz clic en "Buscar" para ver tu historial de
                            comentarios.</p>
                    </div>
                </div>

                <button class="prefcont list-item" id="btn-preferencias">Publicaciones</button>
                <div id="preferencias-section"
                    class="mt-4 hidden bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                    <h4 class="text-md font-bold mb-4">Historial de Publicaciones</h4>
                    <div class="flex border-b">
                        <button id="btn-pref-personal" onclick="cargarPreferencias('personal')"
                            class="flex-1 py-3 text-sm font-bold border-b-2 border-purple-600 text-purple-600 bg-purple-50">Personal</button>
                        <button id="btn-pref-contenido" onclick="cargarPreferencias('contenido')"
                            class="flex-1 py-3 text-sm font-bold border-b-2 border-transparent text-gray-500 hover:bg-gray-50">Contenido</button>
                    </div>
                    <div id="lista-preferencias-contenido"
                        class="max-h-[400px] overflow-y-auto divide-y divide-gray-100"></div>
                </div>
            </div>
            ...
            <div class="select-conf w">
                ...
                <button class="cambNom list-item flex justify-between items-center" id="btn-abrir-editor"> Cambiar
                    nombre de usuario</button>
                <div id="plantilla-editar-perfil"
                    class="hidden mt-2 bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                    <div class="mb-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mb-4">
                        ...
                        <div class="flex flex-col gap-1"><input type="text" id="nuevo-apellido" placeholder="Apellido"
                                class="p-2 bg-gray-50 border border-gray-100 rounded-lg outline-none focus:border-purple-400 text-sm transition-colors">
                        </div>
                    </div>

                    <button onclick="guardarCambiosPerfil()"
                ...
                <div id="plantilla-editar-pass"
                    ...
                    <div class="mb-4">
                        ...
                        <h4>La contraseña debe tener al menos 8 caracteres e incluir la combinación de numeros, letras y
                            caracteres especiales (!$@%). </h4>
                    </div>
                    <div class="flex flex-col md:flex-row gap-4">

                        <input type="password" id="pass-actual" placeholder="Contraseña actual"
                            class="w-full p-2 bg-gray-50 border border-gray-100 rounded-lg outline-none focus:border-purple-400 text-sm transition-colors">
                        <input type="password" id="pass-nueva" placeholder="Nueva contraseña"
                            class="w-full p-2 bg-gray-50 border border-gray-100 rounded-lg outline-none focus:border-purple-400 text-sm transition-colors">
                        <input type="password" id="pass-repetir" placeholder="Repetir nueva contraseña"
                            class="w-full p-2 bg-gray-50 border border-gray-100 rounded-lg outline-none focus:border-purple-400 text-sm transition-colors">
                ...
                <div id="plantilla-editar-correo"
                    class="hidden mt-2 bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                    <div class="mb-4">
                        <label class="text-[10px] font-bold text-purple-500 uppercase">Perfil de:</label>
                        <div class="flex items-center gap-2 mt-1">
                    ...
                    <div class="flex flex-col gap-3">
                        <input type="email" id="correo-nuevo" placeholder="Nuevo correo electrónico"
                            class="p-2 bg-gray-50 border border-gray-100 rounded-lg outline-none focus:border-purple-400 text-sm transition-colors">

                        <button onclick="actualizarCorreo()"
                            class="w-full bg-purple-600 text-white py-2 rounded-lg font-bold hover:bg-purple-700 transition-colors">
                            Actualizar Correo
                        </button>
                    </div>
            ...
            <div class="select-conf bg-white p-4 rounded shadow">
                ...
                <h4 class="text-sm ">
                    Prohibido contenido que promueva el odio, la violencia, el acoso, la humillación o la intimidación.
                    No se permite compartir información privada sin consentimiento para chantaje o acoso.
                    Se prohíben las amenazas a la integridad física y las actividades que fomenten la autolesión.
                    Integridad y Autenticidad de la Cuenta: No suplantar identidades ni crear cuentas falsas.
                    No vender, licenciar o comprar cuentas o datos. Evitar comportamientos de bot, como publicar con
                    demasiada frecuencia. Actividades Ilegales y Peligrosas: No se tolera el apoyo al terrorismo,
                    crimen organizado o grupos de odio. Prohibida la venta de armas, drogas y ofertas de servicios
                    sexuales. Restricciones sobre productos de tabaco y suplementos inseguros. Propiedad Intelectual
                    y Contenido: Respetar los derechos de autor; obtener permiso para republicar contenido y dar crédito
                    al autor original. Contenido explícito debe ser advertido o retirado; se favorece la información
